export class Recipe {
  ...
  set tiempo(aTiempo: number) {
    if (aTiempo == 0) {
      throw new Error("El tiempo debe de ser mayor a cero");
    }
  ...
  toString(): string {
    return `Receta: ${this.name} - categoría: ${this.category} -Descripción:  ${this.descripcion} -  tiempo: ${this.tiempo} minutos`;
  }
}
